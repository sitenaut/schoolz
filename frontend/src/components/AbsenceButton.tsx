...
export function AbsenceButton({ school, className = "action primary" }: { school: School; className?: string }) {
  if (school.absence_method === "email" && school.absence_emails.length > 0) {
    ...
    return (
      <a className={className} href={`mailto:${school.absence_emails.join(",")}?subject=${subject}&body=${body}`}>
        <IconMail />
        Report absence
      </a>
    );
  ...
  if (school.absence_method === "phone" && school.absence_phone) {
    return (
      <a className={className} href={telHref(school.absence_phone)}>
        <IconPhone />
        Report absence
      </a>
    );
  ...
  if (school.absence_method === "portal" && school.absence_portal_url) {
    return (
      <a className={className} href={school.absence_portal_url} target="_blank" rel="noreferrer">
